...
function fixtureEngine(caseId: string): StockfishEngine {
    if (caseId === 'golden-quiet') {
        ...
        return {
            evalPosition: async ({ fen }) =>
                fen === root
                    ? result(fen, 'e2e3', { type: 'cp', value: 100 }, [
                          'e2e3',
                          'e7e5',
                      ])
                    : result(fen, 'e7e5', { type: 'cp', value: 200 }, [
                          'e7e5',
                          'g1f3',
                      ]),
    ...
    if (caseId === 'golden-mate') {
        const root = '7k/5Q2/6K1/8/8/8/8/8 w - - 0 1';
        return {
            evalPosition: async ({ fen }) =>
                fen === root
                    ? result(fen, 'f7f8', { type: 'mate', value: 1 }, ['f7f8'])
                    : result(fen, 'h8g8', { type: 'cp', value: 0 }, ['h8g8']),
            analyzeMultiPv: async ({ fen }) =>
                multi(fen, [
                    {
                        moveUci: 'f7f8',
    ...
    if (caseId === 'golden-repetition') {
        ...
        return {
            evalPosition: async ({ fen }) => {
                if (fen === root) {
                    return result(fen, 'e7e5', { type: 'cp', value: -300 }, [
                        'e7e5',
                        'g1f3',
                    ]);
                }
                if (fen === played) {
                    return result(fen, 'g1f3', { type: 'cp', value: 400 }, [
                        'g1f3',
                        'b8c6',
                    ]);
                }
    ...
    if (caseId === 'golden-en-passant') {
        ...
        return {
            evalPosition: async ({ fen }) =>
                fen === root
                    ? result(fen, 'e5d6', { type: 'cp', value: 200 }, [
                          'e5d6',
                          'c7d6',
                      ])
                    : fen === played
                      ? result(fen, 'c7c5', { type: 'cp', value: 200 }, [
                            'c7c5',
                            'd2d4',
                        ])
    ...
    if (caseId === 'golden-no-error') {
        ...
        return {
            evalPosition: async ({ fen }) =>
                fen === root
                    ? result(fen, 'e2e4', { type: 'cp', value: 0 }, [
                          'e2e4',
                          'e7e5',
                      ])
                    : result(fen, 'e7e5', { type: 'cp', value: 0 }, [
                          'e7e5',
                          'g1f3',
                      ]),
            analyzeMultiPv: async ({ fen }) => multi(fen, []),
        };
    }
...
class CountingEngine implements StockfishEngine {
    ...
    async evalPosition(
        ...
        if (options.rootMoves?.length === 1) {
            const move = options.rootMoves[0]!;
            const child = await this.delegate.evalPosition({
                ...options,
                fen: after(options.fen, move),
                rootMoves: undefined,
            });
            return {
                ...child,
                fen: options.fen,
                bestMoveUci: move,
                pvUci: [move, ...child.pvUci],
                score: negateScore(child.score),
                wdl: reverseWdl(child.wdl),
            };
        }
...
async function runCorpus(): Promise<{
    ...
    for (const fixture of corpus.extractionCases) {
        const engine = new CountingEngine(fixtureEngine(fixture.id));
        const normalizedGame: NormalizedGame = {
            ...
            timeClass: 'rapid',
            white: {
                name: fixture.usernameColor === 'white' ? 'adam' : 'opponent',
            },
            black: {
                name: fixture.usernameColor === 'black' ? 'adam' : 'opponent',
            },
            pgn: fixture.pgn,
            provenance: {
                username: 'adam',
                userSide: fixture.usernameColor === 'black' ? 'black' : 'white',
            },
        };
        const output = await extractTrainingMomentsFromGames({
            ...
            options: {
                ...
                confirmNodes: fixture.id === 'golden-repetition' ? 500 : null,
        ...
        expect(output.moments).toHaveLength(fixture.expectedMoments.length);
...
describe('versioned training V2 golden corpus', () => {
    it('keeps coverage links executable and the PGN corpus stable within its cost budget', async () => {
        ...
        const coverage = new Set([
            ...corpus.extractionCases.flatMap((fixture) => fixture.coverage),
            ...corpus.linkedAssertions.map((link) => link.coverage),
        ]);
        ...
        const expected = corpus.extractionCases.flatMap((fixture) =>
